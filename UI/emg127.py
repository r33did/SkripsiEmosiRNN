...
class Ui_MainWindow(QWidget):
    ...
    def setupUi(self, MainWindow):
        ...
        self.statusValue_2 = QtWidgets.QLabel(self.centralwidget)
        font = QtGui.QFont()
        font.setPointSize(9)
        font.setBold(True)
        font.setWeight(75)
        self.statusValue_2.setFont(font)
        self.statusValue_2.setStyleSheet("color: rgb(255, 255, 255);")
        self.statusValue_2.setAlignment(QtCore.Qt.AlignCenter)
        self.statusValue_2.setObjectName("statusValue_2")
        self.gridLayout_2.addWidget(self.statusValue_2, 8, 0, 1, 2)
...
#=================== EDIT HERE======================================
        ...
        self.startButton.clicked.connect(lambda: self.draw())
        # self.stopButton.clicked.connect(lambda: self.start_button('STOP'))
        self.stopButton.clicked.connect(lambda: self.clearplot())
    ...
    def read_arduino(self,arduino):
        ...
        if sys.getsizeof(c[0])==56:
                Ui_MainWindow.pipi.append(c[0])
                Ui_MainWindow.alis.append(c[1])
                Ui_MainWindow.wkt.append(waktu)
    ...
    def browsefile(self):
        ...
        'E:\Kuliah\Tugas Akhir\Source Code\test_git\SkripsiEmosiRNN\IAPS hari 2','CSV files (*.csv)')
        self.statusValue_2.setText(fname[0])
        self.statusValue_2.adjustSize()
        df = pd.read_csv(str(fname[0]))
        self.statusValue.setText('Loaded !')
        self.statusValue.adjustSize()
        return df
    ...
    def draw(self):
        df1 = pd.read_csv(self.statusValue_2.text())
        pipi = df1['Pipi'].to_numpy()
        alis = df1['Alis'].to_numpy()
        length = len(pipi)
        self.statusValue.setText('Start !')
        self.iterate = [p for p in range(0,length)]
        self.widgetPlot.plot(self.iterate,pipi,pen=('y'),name='EMG Pipi')
        self.widgetPlot.plot(self.iterate,alis,pen=('g'),name='EMG Alis')
        # for j in range(0,length):
        #         self.iterate.append(int(j))
        #         self.pipix.append(pipi[j])
        #         self.alisx.append(alis[j])
        #         self.widgetPlot.plot(self.iterate,self.pipix)
    def update(self):
        global ptr
        self.statusValue.setText('Start !')
        # while (arduinoData.inWaiting()==0):                         #wait until there is data available
        #     pass # do nothing
        drawplot1 = self.widgetPlot.plot(pen=('y'),name = 'EMG Pipi')
        drawplot2 = self.widgetPlot.plot(pen=('b'), name = 'EMG Alis')
        arduinoString = arduinoData.readline()
        b = arduinoString.decode("ISO-8859-1").strip()
        c = b.split(',')
        if sys.getsizeof(c[0])==56:
                self.pipix=np.append(self.pipix,c[0])
                self.alisx=np.append(self.alisx,c[1])
                self.widgetPlot.setData(self.ptr,self.pipix)
        # df1 = pd.read_csv(self.statusValue_2.text())
        # pipi = df1['Pipi'].to_numpy()
        # alis = df1['Alis'].to_numpy()
        # length = len(pipi)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.statusValue_2.setText(_translate("MainWindow", "-"))
